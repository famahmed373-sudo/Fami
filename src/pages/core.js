    ...
    el('div', { class: 'grid2' },
      el('div', { class: 'panel' },
        ...
        recent.length ? el('div', { class: 'tableWrap' }, el('table', { class: 'tbl' },
          el('thead', {}, el('tr', {}, el('th', {}, 'Date'), el('th', {}, 'Tenant'), el('th', { class: 'right' }, 'Amount'), el('th', {}, 'Status'))),
  ...
  const root = el('div', {},
    el('div', { class: 'toolbar' },
      el('div', { class: 'filters' },
        el('div', { class: 'search' }, el('span', { html: iconSvg('search', 16) }), el('input', { placeholder: 'Search tenant, type of work, unit...', oninput: (e) => { q = e.target.value; renderGrid(); } })),
      ...
      can('edit')
        ? el('button', { class: 'btn primary', onclick: () => shopModal(ctx, null) }, el('span', { class: 'ic', html: iconSvg('plus', 15) }), 'Register tenant')
        : el('button', { class: 'btn primary', disabled: true, title: 'Requires Manager or Admin role', onclick: () => toast('Your role is read-only. Ask an admin to upgrade you to Manager or Admin.', 'info') },
            el('span', { class: 'ic', html: iconSvg('plus', 15) }), 'Register tenant')
    ),
...
function shopCard(s, ctx) {
  ...
  return el('div', { class: 'panel shopCard' },
    ...
    el('div', {},
      el('h3', {}, el('span', { html: iconSvg('users', 17), style: { color: 'var(--brand-ink)' } }), esc(s.name)),
    ...
    el('div', { class: 'meta' },
      el('div', {}, 'Type of work: ', s.tenant_name ? el('b', {}, esc(s.tenant_name)) : el('span', { class: 'muted' }, '—')),
...
function shopModal(ctx, shop) {
  ...
  const name = el('input', { placeholder: 'Tenant full name, e.g. Selam Bekele', value: f(shop && shop.name) });
  const unit = el('input', { placeholder: 'e.g. A-01', value: f(shop && shop.unit) });
  const tenantName = el('input', { placeholder: 'e.g. Bakery, Tailoring, Grocery', value: f(shop && shop.tenant_name) });
  ...
  const save = async () => {
    if (!name.value.trim()) return toast('Tenant name is required', 'error');
    ...
    try {
      const saved = await api.saveShop({ id: shop && shop.id, name: name.value, unit: unit.value, tenant_name: tenantName.value, tenant_phone: tenantPhone.value, rent_amount: rent.value, status: status.value, registered_month: regMonth.value, notes: notes.value });
      if (isNew) {
        // Step 2 of registration: proceed straight into recording the first payment.
        const created = Array.isArray(saved) ? saved[0] : saved;
        if (created) FAMI.shops.unshift(created);
        registrationPaymentModal(ctx, created || { id: null, name: name.value.trim(), rent_amount: rent.value, registered_month: regMonth.value });
        return;
      }
      closeModal();
      toast('Shop updated');
  ...
  openModal({
    title: isNew ? 'Register tenant' : `Edit ${shop.name}`,
    sub: isNew ? 'Step 1 of 2 — register the tenant and assign a shop unit.' : 'Update tenant, type of work and photo details.',
    body: el('div', { class: 'formGrid' },
      el('div', { class: 'field full' }, el('label', { html: 'Tenant name <span class="req">*</span>' }), name),
      ...
      el('div', { class: 'field' }, el('label', {}, 'Type of work'), tenantName, el('div', { class: 'hint' }, 'What the tenant runs, e.g. Bakery, Pharmacy, Salon.')),
      ...
      el('div', { class: 'formActions full' },
        ...
        el('button', { class: 'btn primary', onclick: save }, isNew ? 'Save & continue to payment' : 'Save changes')
      )
    )
  });
...
function deleteShopFlow(ctx, shop) {
  confirmDialog({
    title: 'Delete tenant?',
    message: `"${shop.name}" (unit ${shop.unit || '—'}) and its payment history will be permanently removed.`,
    confirmText: 'Delete tenant', danger: true,
    onConfirm: async () => {
      try { await api.deleteShop(shop.id); toast('Tenant deleted'); await ctx.refresh(); }
      catch (e) { toast(e.message || 'Could not delete shop', 'error'); }
    }
  });
}

// First day / last day of a 'YYYY-MM' month, as 'YYYY-MM-DD' strings.
const monthRange = (m) => {
  const [y, mo] = String(m).split('-').map(Number);
  const last = new Date(y, mo, 0).getDate();
  return { from: `${m}-01`, upto: `${m}-${String(last).padStart(2, '0')}` };
};

// Step 2 of tenant registration: record the first payment so the
// registration is finished in the same flow.
function registrationPaymentModal(ctx, shop) {
  const { from: defFrom, upto: defUpto } = monthRange(shop.registered_month || monthKey());
  const from = el('input', { type: 'date', value: defFrom });
  const upto = el('input', { type: 'date', value: defUpto });
  const amount = el('input', { type: 'number', min: 0.01, step: 0.01, value: shop.rent_amount || '' });
  ...
  const finish = async () => {
    if (!from.value || !upto.value) return toast('Enter the payment period (from and up to dates)', 'error');
    if (from.value > upto.value) return toast('The "up to" date must be after the "from" date', 'error');
    if (!(Number(amount.value) > 0)) return toast('Enter a valid amount', 'error');
    try {
      await api.addPayment({ shop_id: shop.id, month: from.value.slice(0, 7), amount: amount.value, date: date.value, method: method.value, reference: ref.value, note: note.value, period_from: from.value, period_upto: upto.value });
      closeModal();
      toast('Tenant registered — first payment recorded');
      await ctx.refresh();
    } catch (e) { toast(e.message || 'Could not record payment', 'error'); }
  };
  const skip = async () => { closeModal(); toast('Tenant registered'); await ctx.refresh(); };

  openModal({
    title: `Finish registration — ${shop.name}`,
    sub: 'Step 2 of 2 — record the first rent payment to complete the registration.',
    body: el('div', { class: 'formGrid' },
      el('div', { class: 'field' }, el('label', { html: 'From date <span class="req">*</span>' }), from),
      el('div', { class: 'field' }, el('label', { html: 'Up to date <span class="req">*</span>' }), upto),
      el('div', { class: 'field' }, el('label', { html: 'Amount (ETB) <span class="req">*</span>' }), amount),
      ...
      el('div', { class: 'hint', style: { gridColumn: '1/-1' } }, 'The payment covers the period above. You can also skip this step and record the payment later.'),
      el('div', { class: 'formActions full' },
        el('button', { class: 'btn', onclick: skip }, 'Skip for now'),
        el('button', { class: 'btn success', onclick: finish }, el('span', { class: 'ic', html: iconSvg('check', 15) }), 'Record payment & finish')
      )
    )
  });
}
...
function paymentForm(preselectShopId) {
  ...
  const shopSel = el('select', {}, shops.map((s) => el('option', { value: s.id, selected: preselectShopId === s.id }, `${s.name}${s.tenant_name ? ' — ' + s.tenant_name : ''}${s.unit ? ' (' + s.unit + ')' : ''}`)));
  const { from: defFrom, upto: defUpto } = monthRange(monthKey());
  const from = el('input', { type: 'date', value: defFrom });
  const upto = el('input', { type: 'date', value: defUpto });
  ...
  const suggest = () => {
    ...
    const a = api.computeArrears(s);
    const period = from.value && upto.value ? `${fmtDate(from.value)} to ${fmtDate(upto.value)}` : monthLabel(monthKey());
    hint.textContent = a.total > 0 ? `${s.name}: ${fmtMoney(a.total)} arrears — this payment covers ${period}.` : `${s.name}: suggested rent ${fmtMoney(s.rent_amount)} for ${period}.`;
  };
  shopSel.addEventListener('change', suggest);
  from.addEventListener('change', suggest);
  upto.addEventListener('change', suggest);

  const submit = async () => {
    const s = FAMI.shops.find((x) => x.id === shopSel.value);
    if (!s) return toast('Select a tenant', 'error');
    if (!from.value || !upto.value) return toast('Enter the payment period (from and up to dates)', 'error');
    if (from.value > upto.value) return toast('The "up to" date must be after the "from" date', 'error');
    if (!(Number(amount.value) > 0)) return toast('Enter a valid amount', 'error');
    try {
      await api.addPayment({ shop_id: s.id, month: from.value.slice(0, 7), amount: amount.value, date: date.value, method: method.value, reference: ref.value, note: note.value, period_from: from.value, period_upto: upto.value });
      toast(`Payment of ${fmtMoney(amount.value)} recorded`);
      amount.value = ''; ref.value = ''; note.value = '';
      from.value = defFrom; upto.value = defUpto;
      await FAMI._refresh();
    } catch (e) { toast(e.message || 'Could not record payment', 'error'); }
  };
  ...
  return el('div', { class: 'panel' },
    ...
    el('div', { class: 'formGrid' },
      el('div', { class: 'field' }, el('label', {}, 'Tenant <span class="req">*</span>'), shopSel),
      el('div', { class: 'field' }, el('label', { html: 'From date <span class="req">*</span>' }), from),
      el('div', { class: 'field' }, el('label', { html: 'Up to date <span class="req">*</span>' }), upto),
...
function historyPanel(params) {
  let q = '', month = 'all', from = '', upto = '', showRev = params.reversed === '1';
  ...
  const filterList = () => FAMI.payments
    .filter((p) => (!p.reversed || showRev) && (month === 'all' || p.month === month) &&
      (!from || p.date >= from) && (!upto || p.date <= upto) &&
  ...
  const renderList = () => {
    ...
    wrap.replaceChildren(list.length ? el('div', { class: 'tableWrap' }, el('table', { class: 'tbl' },
      el('thead', {}, el('tr', {}, el('th', {}, 'Date'), el('th', {}, 'Tenant'), el('th', {}, 'Period'), el('th', { class: 'right' }, 'Amount'), el('th', {}, 'Method'), el('th', {}, 'Reference'), el('th', {}, 'Status'), el('th', {}, 'By'), el('th', { class: 'right' }, 'Actions'))),
  ...
  const root = el('div', { class: 'panel' },
    ...
    el('div', { class: 'toolbar', style: { marginTop: 0 } },
      el('div', { class: 'filters' },
        el('div', { class: 'search' }, el('span', { html: iconSvg('search', 16) }), el('input', { placeholder: 'Search tenant, type of work, reference...', oninput: (e) => { q = e.target.value; renderList(); } })),
        ...
        el('input', { type: 'date', value: from, title: 'Filter: from payment date', 'aria-label': 'From date', oninput: (e) => { from = e.target.value; renderList(); } }),
        el('input', { type: 'date', value: upto, title: 'Filter: up to payment date', 'aria-label': 'Up to date', oninput: (e) => { upto = e.target.value; renderList(); } }),
...
// The period a payment covers: 'from – upto' when recorded with dates, else the month.
const periodLabel = (p) => (p && p.period_from && p.period_upto ? `${fmtDate(p.period_from)} – ${fmtDate(p.period_upto)}` : monthLabel(p && p.month));
...
function exportPayments(list) {
  downloadCSV(`fami-payments-${today()}.csv`, [
    ['Date', 'Tenant', 'Type of work', 'Period from', 'Period up to', 'Amount', 'Method', 'Reference', 'Note', 'Status', 'Recorded by'],
    ...list.map((p) => [p.date, shopName(p.shop_id), tenantOf(p.shop_id), p.period_from || '', p.period_upto || '', p.amount, p.method, p.reference, p.note, p.reversed ? 'reversed' : 'posted', byName(p.user_id)])
  ]);
}
